...
export const MessageBlock = React.memo(() => {
	...
	const messages = messagesSheet.map(el => {
		...
		else if (el.friendMessageText !== '') return <FriendMessage key={el.id} message={el.friendMessageText || ''} userLogo={10} />
	})

	// Прибитие диалоговых сообщений к низу при открытии диалога
